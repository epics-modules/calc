...
MAIN(scalcTest)
{
	...
	testValExpr("!A", args, sargs, not A);
	...
	testValExpr("B?0:!A", args, sargs, B ? 0 : (not A));
	testValExpr("1", args, sargs, 1);
	testValExpr("A?0:B", args, sargs, A ? 0 : B);
	testValExpr("A&&B&&!I", args, sargs, A && B && (not I));
	...
	testValExpr("A=1", args, sargs, A == 1);
	testValExpr("A&(I||!J)&(K||!L)", args, sargs, (int) A & (int) (I || not J) & (int) (K || not L));
	testValExpr("(A||!B)&(C||!D)&(E||!F)&(G||!H)", args, sargs, (int) (A || not B) & (int) (C || not D) & (int) (E || not F) & (int) (G || not H));
